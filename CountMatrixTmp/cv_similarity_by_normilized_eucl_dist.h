...
namespace cm
{
	...
	public:
		...
		{
			const distance_between_cv<double> normilized_distance = cv_normilized_euclidean_distance<CountVectorDimension>::calculate(first_vector, second_vector);
			...
			return distance_between_cv(std::clamp(similarity, min_value_, max_value_));
		}
	};
}