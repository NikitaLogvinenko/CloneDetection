...
namespace clang_code_analysis
{
	...
	public:
		...
		void analyse(const func_id analyzed_id, const CXCursor& nested_cursor, const var_usage_callback& callback) override
		{
			...
			const finding_nth_nested_variable_data data{ analyzed_id, std::ref(callback), std::ref(*this) };
	...
	private:
		...
		{
			const auto data_ptr = static_cast<const finding_nth_nested_variable_data*>(data_void_ptr);
