...
namespace clang_code_analysis
{
	...
	requires (cm::count_vector_length<ConditionsCount> && std::predicate<Predicate, const CXCursor&>)
	class vars_nested_in_condition_finder final
	...
	private:
		...
		struct finding_all_nested_variables_data final
		{
			...
			const vars_nested_in_condition_finder& this_ref;
		};

		Predicate predicate_{};

	public:
		vars_nested_in_condition_finder() noexcept(std::is_nothrow_default_constructible_v<Predicate>) = default;

		explicit vars_nested_in_condition_finder(std::vector<condition_index> indices, Predicate predicate)
		...
		void analyse(const func_id analyzed_id, const CXCursor& nested_cursor, const var_usage_callback& callback) override
		{
			...
			finding_all_nested_variables_data data{ analyzed_id, callback, *this };
