...
#pragma pack(push, 1)
	struct BitmapHeader
	{
		...
		std::uint16_t planes;             /* Number of clip planes */
...
#pragma pack(pop)
	...
	public: /* Accessors */
		...
		/**
		 *	Clears Bitmap pixels with an rgb clip
		 */
	...
	public: /* Modifiers */
		/**
		 *	Sets rgb clip to pixel at position x,y
			...
			if (std::ifstream ifs{ filename, std::ios::binary })
			{
				...
				for (std::int32_t y = m_height - 1; y >= 0; --y)
				{
					...
					for (std::int32_t x = 0; x < m_width; ++x)
					{
						...
						color.g = line[i++];
						color.r = line[i++];
						m_pixels[IX(x, y)] = color; // this->Set(x, y, clip);
					}
				}
