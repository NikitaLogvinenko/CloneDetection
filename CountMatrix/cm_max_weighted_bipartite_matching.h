...
private:
	...
	[[nodiscard]] std::vector<cm_similar_vectors> create_similar_vectors_pairs(
	...
	const dlib::matrix<size_t>& assignment_matrix) const
{
	...
	std::vector<cm_similar_vectors> similar_vectors{};
	if (first_cm.vectors_count() < second_cm.vectors_count())
	{
...
template <size_t CountVectorDimension>
std::vector<cm_similar_vectors> cm_max_weighted_bipartite_matching<CountVectorDimension>::create_similar_vectors_pairs(
