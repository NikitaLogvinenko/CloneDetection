...
TEST(cm_similar_vectors_data_test, ctor_default)
{
	...
	ASSERT_EQ(default_data.index_of_first_vector().to_size_t(), cm::index_of_count_vector{}.to_size_t());
	ASSERT_EQ(default_data.index_of_second_vector().to_size_t(), cm::index_of_count_vector{}.to_size_t());
	ASSERT_EQ(default_data.relative_similarity(), cm::relative_similarity{});
}

TEST(cm_similar_vectors_data_test, ctor_with_args)
{
	const cm::index_of_count_vector first_index{1};
	const cm::index_of_count_vector second_index{2};
