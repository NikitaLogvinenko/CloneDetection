...
protected:
	...
	count_matrix<3> cm_1_{};
	count_matrix<3> cm_2_{};
	count_matrix<3> cm_2_incomplete_{};
	count_matrix<3> cm_1_repeated_cv_{};
	count_matrix<3> cm_2_repeated_cv_{};

	count_matrix<3> cm_1_copy_{};
	std::vector<count_vector<3>> count_vectors_1_reversed_vectors_order_{};
	count_matrix<3> cm_1_reversed_vectors_order_{};

	count_matrix<3> cm_empty_1_{};
	count_matrix<3> cm_empty_2_{};
	...
	void SetUp() override
	{
		...
		cm_1_ = count_matrix{count_vectors_1_};

		count_vectors_2_ = std::vector{cv_1_2_, cv_2_2_, cv_3_2_, cv_4_2_};
		cm_2_ = count_matrix{count_vectors_2_};

		count_vectors_2_incomplete_ = std::vector{cv_2_2_, cv_3_2_, cv_4_2_};
		cm_2_incomplete_ = count_matrix{count_vectors_2_incomplete_};

		count_vectors_1_repeated_cv_ = std::vector{
			cv_1_1_, cv_1_1_, cv_2_1_, cv_2_1_, cv_3_1_, cv_3_1_, cv_4_1_, cv_4_1_
		};
		cm_1_repeated_cv_ = count_matrix{count_vectors_1_repeated_cv_};

		count_vectors_2_repeated_cv_ = std::vector{
			cv_1_2_, cv_1_2_, cv_2_2_, cv_2_2_, cv_3_2_, cv_3_2_, cv_4_2_, cv_4_2_
		};
		cm_2_repeated_cv_ = count_matrix{count_vectors_2_repeated_cv_};

		cm_1_copy_ = cm_1_;
		std::ranges::copy(count_vectors_1_.rbegin(), count_vectors_1_.rend(),
		                  std::back_inserter(count_vectors_1_reversed_vectors_order_));
		cm_1_reversed_vectors_order_ = count_matrix{count_vectors_1_reversed_vectors_order_};
	}
};
...
TEST_F(cm_max_weighted_bipartite_matching_dim3_test_f, two_empty_matrices)
{
	...
	ASSERT_EQ(cm_similarity_data.matrices_relative_similarity().to_double(),
...
TEST_F(cm_max_weighted_bipartite_matching_dim3_test_f, two_identical_matrices)
{
	...
	for (const auto& similar_cv_data : cm_similarity_data)
	{
		...
		ASSERT_EQ(similar_cv_data.index_of_first_vector().to_size_t(),
		          similar_cv_data.index_of_second_vector().to_size_t());
	}
}
...
TEST_F(cm_max_weighted_bipartite_matching_dim3_test_f, equal_vectors_but_different_order)
{
	...
	for (const auto& similar_cv_data : cm_similarity_data)
	{
		ASSERT_EQ(similar_cv_data.relative_similarity().to_double(), relative_similarity::max_similarity_value());
		ASSERT_EQ(
			similar_cv_data.index_of_first_vector().to_size_t() + similar_cv_data.index_of_second_vector().to_size_t(),
			cm_1_.vectors_count() - 1);
	}
}
...
TEST_F(cm_max_weighted_bipartite_matching_dim3_test_f, different_matrices_equal_cv_count)
{
	...
	const std::vector cv_similarities_descending_order{
		vectors_similarity_4, vectors_similarity_3, vectors_similarity_2, vectors_similarity_1
	};

	const double cm_similarity = (vectors_similarity_1 + vectors_similarity_2 + vectors_similarity_3 +
		vectors_similarity_4) / 4;
	...
	{
		const auto& cv_pair = cm_similarity_data.get_similar_vectors_data_dont_check_index(
			index_of_count_vectors_pair(cv_pair_index));
...
{
	constexpr double distance_1 = 1.0;
	// between first vector in cm_1_ and fourth (fictive) zero-vector in cm_2_incomplete_
	...
	const double vectors_similarity_4 = 1 - distance_4;
	const std::vector cv_similarities_descending_order{
		vectors_similarity_4, vectors_similarity_3, vectors_similarity_2
	};
...
TEST_F(cm_max_weighted_bipartite_matching_dim3_test_f, several_possible_matchings)
{
	...
	const std::vector cv_similarities_descending_order{
		vectors_similarity_78, vectors_similarity_78, vectors_similarity_56, vectors_similarity_56,
		vectors_similarity_34, vectors_similarity_34, vectors_similarity_12, vectors_similarity_12
	};

	const double cm_similarity = (vectors_similarity_12 + vectors_similarity_34 + vectors_similarity_56 +
		vectors_similarity_78) / 4;
...
protected:
	...
	count_matrix<0> cm_four_cv_{};
	count_matrix<0> cm_three_cv_{};

	count_matrix<0> cm_empty_1_{};
	count_matrix<0> cm_empty_2_{};

	void SetUp() override
	{
		four_count_vectors_ = std::vector{cv_, cv_, cv_, cv_};
		cm_four_cv_ = count_matrix{four_count_vectors_};

		three_count_vectors_ = std::vector{cv_, cv_, cv_};
		cm_three_cv_ = count_matrix{three_count_vectors_};
	}
};
...
TEST_F(cm_max_weighted_bipartite_matching_zero_dim_test_f, matrices_with_different_count_of_vectors)
{
	...
	ASSERT_NEAR(cm_similarity_data.matrices_relative_similarity().to_double(),
	            relative_similarity::max_similarity_value(), 1e-3);
...
TEST_F(cm_max_weighted_bipartite_matching_dim3_test_f,
       different_matrices_equal_cv_count_swap_matrices_through_abstract_class)
{
	const double distance_1 = std::sqrt(3.0 / 14);
	const double distance_2 = std::sqrt(3.0 / 77);
	const double distance_3 = std::sqrt(3.0 / 194);
