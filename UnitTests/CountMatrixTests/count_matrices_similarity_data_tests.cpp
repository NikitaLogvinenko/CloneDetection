...
TEST(count_matrices_similarity_data_test, ctor_initial_descending_order)
{
	...
	for (size_t initial_pair_index = 0; initial_pair_index < similar_vectors_data_descending_order.size(); ++
	     initial_pair_index)
	{
		const auto& pair_from_vector = similar_vectors_data_descending_order[initial_pair_index];
		const auto& pair_from_cm_sim_data = cm_similarity_data.get_similar_vectors_data_dont_check_index(
			index_of_count_vectors_pair(initial_pair_index));
		assert_equal_cm_similar_vectors_data(pair_from_vector, pair_from_cm_sim_data);
	}
}
...
TEST(count_matrices_similarity_data_test, iterators)
{
	...
	{
		const auto& pair_by_index = cm_similarity_data.get_similar_vectors_data_dont_check_index(
			index_of_count_vectors_pair(forward_index));
	...
	for (auto iterator = cm_similarity_data.end(); iterator > cm_similarity_data.begin();)
	{
		--iterator;
		const auto& pair_by_index = cm_similarity_data.get_similar_vectors_data_dont_check_index(
			index_of_count_vectors_pair(backward_index));
		assert_equal_cm_similar_vectors_data(*iterator, pair_by_index);
		--backward_index;
	}
