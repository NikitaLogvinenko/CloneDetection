...
{
	constexpr cm::relative_similarity default_relative_similarity{};
...
TEST(relative_similarity_test, ctor_with_valid_similarity)
{
	...
	const cm::relative_similarity valid_relative_similarity{valid_value};
	ASSERT_EQ(valid_relative_similarity.to_double(), valid_value);
}

TEST(relative_similarity_test, ctor_with_invalid_similarity)
{
	ASSERT_THROW(cm::relative_similarity(-0.1), common_exceptions::initialized_value_out_of_range);
	ASSERT_THROW(cm::relative_similarity(1.1), common_exceptions::initialized_value_out_of_range);
}

TEST(relative_similarity_test, comparing_operators)
{
	const cm::relative_similarity small_similarity{0.1};
	const cm::relative_similarity small_similarity_copy(small_similarity);
	const cm::relative_similarity big_similarity{0.9};
...
{
	ASSERT_EQ(cm::relative_similarity::min_similarity_value(), 0);
	ASSERT_EQ(cm::relative_similarity::max_similarity_value(), 1);
}
