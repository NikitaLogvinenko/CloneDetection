...
namespace code_analysis_through_cm
{
	...
	class code_traverser_sharing_units : public code_traverser_abstract<AnalysisTraits>
	{
	public:
	...
	protected:
		code_traverser_sharing_units(std::shared_ptr<translation_units_container> not_traversed_units, 
		...
		{
			const auto method_name = "code_traverser_sharing_units::code_traverser_sharing_units";
		...
		virtual void traverse_unit(std::unique_ptr<TranslationUnitT> translation_unit, const nested_entity_condition_callback<AnalysisTraits>& callback) = 0;
